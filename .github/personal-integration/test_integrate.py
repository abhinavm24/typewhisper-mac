...
class SelectionTests(unittest.TestCase):
    ...
    def test_draft_lookup_uses_release_id_not_published_tag_endpoint(self):
        result = subprocess.CompletedProcess([], 0, '{"databaseId": 123}', "")
        with patch.object(m, "run", return_value=result), patch.object(m, "api", return_value={"draft": True}) as api:
            self.assertTrue(m.release_by_tag("personal-example")["draft"])
            api.assert_called_once_with(f"repos/{m.REPO}/releases/123")
...
class GitAssemblyTests(unittest.TestCase):
    ...
    def publication_fixture(self):
        ...
        def command(*args, **kwargs):
            ...
            operation = args[2]
            if operation == "view":
                return subprocess.CompletedProcess(args, 0 if state["release"] else 1, '{"databaseId": 7}', "")
            if operation == "create":
                state["release"] = {"id": 7, "draft": True, "prerelease": True, "assets": [],
                                    "tag_name": manifest["tag"],
                                    "body": f"<!-- personal-inputs:{manifest['fingerprint']} -->",
        ...
        def api(endpoint, pages=False):
            ...
            if endpoint != f"repos/{m.REPO}/releases/7":
                raise RuntimeError("Not Found (HTTP 404): drafts require their numeric release ID")
            return copy.deepcopy(state["release"])
        self.enterContext(patch.object(m, "api", side_effect=api))
        return root, source, assets, manifest, state, remote

    def test_second_concurrent_run_skips_already_published_inputs(self):
        ...
        manifest["tag"] = "personal-20260925-130000-run2-attempt1"
        (source / "integration-manifest.json").write_text(json.dumps(manifest))
        m.publish(SimpleNamespace(control=self.path, source=source, assets=assets, work=root / "work2"))
        self.assertFalse((root / "work2").exists())
        self.assertEqual(m.git(remote, "tag", "--list"), "personal-20260925-120000-run1-attempt1")
